...
# Ensure that the tree-sitter dynamic library is generated and points to the correct path
...
def sanitize(code: str, entrypoint: Optional[str] = None) -> str:
    ...
    if not tree.root_node:
        print("Parsed tree root node is empty")
        return ""
    ...
    if not sanitized_output:
        print("Sanitized output is empty")
        return ""
...
def script(
    ...
    if not inplace:
        if is_folder:
            new_name = target_path.name + "-sanitized-mod"
        else:
            new_name = target_path.name.replace(".jsonl", "-sanitized-mod.jsonl")
    ...
    for solution in tqdm(load_solutions(samples)):
        ...
        if task_id not in dataset:
            print(
                f"Skipping {task_id} as it does not exist in the latest EvalPlus dataset."
            )
            continue
        ...
        new_solutions.append({"task_id": task_id, "completion": new_code})

        # Write to file immediately after generating each new solution
        write_jsonl(target_path, [new_solutions[-1]], append=True)
    ...
    if nsan > 0:
        ...
        print(f"All files seem valid -- no files are sanitized.")
